...
def submit_registry():
    loggedin_guest_id = session.get('guest')
    if loggedin_guest_id is None:
        return redirect('/login')

    guest = Guests.query.get(loggedin_guest_id)
    if guest is None:
        return redirect('/login')

    if request.method == 'POST':
        selected_gift_ids = request.form.getlist('gifts')
        guest_gifts = Guest_gift.query.filter_by(g_guestid=loggedin_guest_id).all()
        for gift in guest_gifts:
            db.session.delete(gift)
        db.session.commit()

        for gift_id in selected_gift_ids:
            guest_gift = Guest_gift(g_giftid=gift_id, g_guestid=loggedin_guest_id)
            db.session.add(guest_gift)
        db.session.commit()
